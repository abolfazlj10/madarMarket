...
// فرض می‌کنیم نوع داده‌ها مطابق با نیاز شما تعریف شده‌اند
...
const Orders = () => {
    ...
    // تابع کمکی برای فرمت‌دهی خوانا و افزودن واحد تومان
    const formatPrice = (price: number) => {
        return `${price.toLocaleString('fa-IR')} تومان`;
    };

    // تابع برای محاسبه مجموع قیمت کل یک سفارش
    const calculateTotalPrice = (items: product[]) => {
        const total = items.reduce((sum, item) => sum + (item.price * item.quantity), 0);
        return formatPrice(total);
    };

    // اگر سفارشی وجود نداشت، پیامی نمایش داده می‌شود
    if (orders.length === 0) {
        return (
