                ...
                {data ? (
                    data.map((item: categories, index: number) => (
                        <SwiperSlide key={index} className="space-y-2 relative mt-2">
                            {/* Compare numbers with numbers now */}
                            <div className={`bg-gradient-to-b from-[#FFF7F5] to-[#FFEBE5] w-18 py-2 px-2 rounded-lg ${item.id === numericCategoryId && 'border border-mainColor'}`}>
                                ...
                                <Link to={`/product/${item.id}`}><img src={`http://localhost:3000/categories/${item.image}`} className="w-22 h-18" /></Link>
                            </div>
                            <Link to={`/product/${item.id}`} className={`text-[#787471] whitespace-nowrap text-center ${item.id === numericCategoryId && 'font-bold'}`}>{item.name}</Link>
                        </SwiperSlide>
                    ))
                ) : (
                    ["", "", "", "", "", ""].map((_, idx) => (
